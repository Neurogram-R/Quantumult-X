...
if (service == "YouTube") {
  ...
  if (url.match(patt)) $done({ body: body })

  if (url.match(/&tlang=/)) $done({ body: body })
...
if (setting.type == "Disable") $done({ body: body })

if (setting.type != "Official" && url.match(/\.m3u8/)) $done({ body: body })
...
if (setting.type == "Official" && url.match(/\.m3u8/)) {
  ...
  if (body.match(patt)) {
    ...
    $task.fetch(options).then(response => {
      ...
      $done({ body: body })
    }, reason => { $done({ body: body }) })

  }

  if (!body.match(patt)) $done({ body: body })
}
...
if (url.match(/\.vtt/) || service == "Netflix") {
  ...
  if (setting.type == "Official") {
    if (subtitles_urls_data == "null") $done({ body: body })
